...
app = FastAPI(title="Ollama Universal SuperAgent", version="1.0.0")
app.add_middleware(
    CORSMiddleware,
...
async def root():
    return {
        "name":"Ollama Universal SuperAgent",
        "status":"online",
...
async def health():
    try:
        ...
        return {"ok":True,"ollama":True,"models":[m.get("name") for m in models.get("models",[])]}
...
async def run_agent(req: AgentRequest):
    try:
        return await agent.run(
            prompt=req.prompt, model=req.model, session_id=req.session_id,
            mode=req.mode, context=req.context, allow_tools=req.allow_tools
        )
    except Exception as e:
...
async def openai_chat(req: OpenAIChatRequest):
    if req.stream:
        raise HTTPException(400, "Streaming is not enabled in v1.0. Use stream=false.")
